...
项目：碱性 OER 微观动力学建模与 FTacV 参数反演平台
负责人：刘拾玉

<!-- STATUS:BEGIN 由 scripts/refresh_status_header.py 生成，请勿手改 -->

| 机器生成字段 | 值 |
|---|---|
| 最后核实 | 2026-09-03 |
| 分支 | `claude/project-review-direction-aa3981` |
| 生成时 HEAD | `bb2ba01` docs(experiment): add step-by-step operating manual |
| 收集到的测试数 | unknown |
...
本文件是**交接文档**，读者是几个月后的自己或接手的人。因此分成两个区，
写法完全不同：

| 区 | 内容 | 规则 |
|---|---|---|
| **当前态**（本节到「当前主线」为止） | 现在是什么情况 | **只改不增**。情况变了就改写原文，不要在后面追加"其实现在不是这样了" |
| **历史日志**（§1 起） | 当时做了什么、当时怎么判断 | **只增不改**。已写的段落不修改，结论作废时在原段落顶部加作废横幅并指向新结论 |

写入要求：

1. **机器生成字段不手写**——HEAD、分支、测试数由 `scripts/refresh_status_header.py`
   生成，改完代码重跑一次。`python/tests/test_status_header.py` 会校验。
2. **每个数字要能追溯**——写"通过 N 项测试""偏差 X%"时必须给出产生它的命令或
   证据文件路径，否则不要写。
3. **结论作废要留痕不要删除**——照 §15 的做法在原段落顶部加横幅，写明作废原因和
   替代结论的位置。删除会让后来者无法判断哪些下游结论受影响。
4. **区分实测与推测**——写判断时标明依据是实测数字、文献还是推断。
   `docs/项目纠错.md` §19 第一版就是把推测写成了结论。
5. **跨文档引用写全称**——写 `WORK_STATUS §19` 或 `纠错 §19`，不要写裸 `§19`：
   两个文档的 §19 内容完全无关。
6. **新增历史小节必须带日期**，格式 `## N. 标题（YYYY-MM-DD）`。

## 当前主线

低维 Bonke 分子催化模型（4 自由参数）的贝叶斯反演。

> **注意**：本文件 2026-08-22 之前的「当前主线」写的是"AEM 热力学描述符的
> 稳定识别"——该结论已于 2026-08-22 撤回（`docs/项目纠错.md` §10：跨数据 CV
> 在 linear/log 混合参数化下不是可辨识性判据）。5 步 AEM 模型降级为待更多
> 数据支持的扩展模型。

当前阶段目标：产出可信后验（含 R-hat 与参数相关矩阵）→ 据此确定补充实验的
频率组与电位窗口（`docs/experiment_plan_2026-08.md` C 节）→ 进实验室。

---

## 1. 当前判断（2026-07-18，历史快照）

> **状态：已过期。** 本节是 2026-07-18 的判断，其结论"多组数据支持 AEM 热力学
> 描述符的稳定识别"已于 2026-08-22 撤回（`docs/项目纠错.md` §10）。
> 当前判断见文件顶部「当前主线」。本节保留供追溯。
...
## 6. 验证状态快照（2026-07-18，历史）

> **状态：已过期。** 当时为 17 passed；当前测试数见文件顶部机器生成的
> STATUS 块。本节保留供追溯。
...
## 7. Git 状态快照（2026-07-18，历史）

> **状态：已过期。** 当时 HEAD 为 `3edb8de`；当前 HEAD 见文件顶部机器生成的
> STATUS 块。本节保留供追溯。
...
## 20. 主线切换与外部校验：从 AEM 反演到低维模型（2026-08-22 至 2026-08-23）

### 20.1 发生了什么

外部审查发现此前主线的核心结论不成立，主线随之切换。本节记录全过程；
所有被撤回的内容保留在原处并加了作废横幅，未删除。

**撤回的结论**：跨数据 CV 显示"AEM 热力学描述符稳定"（本文件 §8/§10.2）。
按 `DEFAULT_PARAM_SPECS` 的实际边界做零信息零假设后，`k0` 的 CV(1.0–1.7)
与纯先验抽样(1.605)在数值上不可区分，而三个"stable"参数相对先验的收缩
倍数几乎相同(2.6–2.8x)。CV=0.3 这个门槛区分的其实是 linear 边界与 log10
边界，不是可辨识性。详见 `docs/项目纠错.md` §10。

同时发现三处支撑该结论的实现缺陷：Tafel 目标在四组数据上都是写死的
60 mV/dec（§11）、Layer 2 的预氧化边界从未被读取（§12）、Bonke 2016 的
引用与该文实际内容不符（§13）。

### 20.2 当前主线

低维 Bonke 分子催化模型：一个表面限域氧化还原耦合一个赝一级催化步，
自由参数四个（`E0_eff`、`k0`、`kf`、`gamma`）。

选择依据是文献的实际拟合自由度——Bonke 2016（同体系唯一先例）用 3 参数；
Gundry 2021 在干净体系的 6 参数模型上仍无法辨识 `Ru` 与 `D`，必须钉住。
本项目现有 4 组数据、各单频、无重复，信息量不支持 8–9 个自由参数。

模型无关的支撑证据：四组数据的 H3 包络峰位为 1.600 ± 0.020 V，H4 伴峰
1.541 ± 0.017 V，跨 2 个频率、3 个扫描窗口、5 倍扫速差一致。

### 20.3 本阶段最重要的结果：Ru 是阻断项

正式反演的 Ru 敏感性（走 HarmPer 优化路径，**不受 §25 的似然缺陷影响**）：

| 数据 | Ru 5→20 Ω 的后果 | ΔHarmPer |
|---|---|---:|
| FT2 | E⁰_eff +274 mV，k₀ 变 14×，γ 变 5.0× | 0.017 |
| FT3 | E⁰_eff +154 mV，k₀ 变 137×，γ 变 0.3× | 0.035 |
| FT4 | E⁰_eff +237 mV，k₀ 变 8×，γ 变 34.9× | 0.071 |
| FT8 | E⁰_eff +349 mV，k₀ 变 115×，γ 变 6.6× | 0.016 |

四组一致：**拟合质量几乎不变，而机理参数相差一到两个数量级。**
即数据区分不出 `Ru` 的取值，但 `Ru` 的取值决定反演出的机理参数。

结论：钉住 `Ru = 10 Ω` 不是"给结果加了误差棒"，是**直接决定了答案**。
EIS 实测 `Ru` 已升级为正式反演的前置阻断项
（`docs/experiment_plan_2026-08.md` A3、`docs/项目纠错.md` §15）。

证据：`results/low_dim_bonke/FT*/ru_sensitivity.csv`

### 20.4 外部校验

项目自建了正演、谐波提取、目标函数与采样器，此前只有自洽测试。
本阶段补了两项外部参照，记录在 `docs/external_validation.md`：

- **采样器 vs PINTS `HaarioACMC`**（Gundry 2021 与 Dale-Evans 2021 用的库）：
  同一解析已知的相关高斯后验，两个实现差异全在蒙特卡洛噪声内，
  无系统偏差。
- **正演 vs 解析闭式解**：可逆极限下 H1–H4 与 Nernst 闭式解相差
  0.04%–0.95%；催化步与稳态闭式解相差 <2%；零法拉第极限退化为纯电容，
  偏差 <0.1%。已固化为 `python/tests/test_analytic_limits.py`。

未采用 MECSim 做数值对照，理由见 `external_validation.md` §2.4：
二进制编译于 2019、仓库停更 7 年、无源码、依赖 Debian 已下架的
`libgfortran3`，要跑通须装配已不存在的运行时——参照物可信度会低于
被验证对象。

### 20.5 加速：可行但未通过门控

C Crank-Nicolson 求解器（`cpp/mc_cn_solver.c`）实测端到端 126×（拯救者）
/ 64×（本机），H2–H4 误差 2–5e-5，两机误差值三位有效数字一致。

但跨参数空间等价性门**两轮均 FAIL**（`results/solver_gate/`）。第二轮
24 次失败全部含相位、无一行单纯幅值失败，集中在小 `gamma` 区
（该区法拉第信号仅比谐波提取本底高约 8 倍）。按预注册规则不调阈值，
**CN 不进入生产**，正式反演用 LSODA。详见 `docs/项目纠错.md` §23 与
`docs/solver_acceleration_feasibility.md` §10。

首轮失败暴露的配置错误已独立修复并保留：CN 步长原按"每周期点数"设定，
而快时间尺度在绝对时间上，改为 `cn_max_dt` 后 `total_nrmse` 最大值由
8.04e-2 降到 2.56e-3。
...
| 项 | 影响 | 处置 |
|---|---|---|
| `Ru` 未测量（§15） | **阻断**——决定机理参数绝对值 | EIS，10 分钟 |
| 参比电极未记录（§16） | 所有电位量整体平移未知常数 | 补记录 |
| 高次谐波受 ADC 量化限制（§14） | H4–H7 信噪比 0.7–5.7 | 降载量 + 收紧量程 |
| 目标函数无外部参照 | BIOMEC 受阻（需 sudo + 容器） | 待定 |
| 准可逆 + 中等 Ru 区无解析参照 | 该区正演未经外部验证 | 待定 |
