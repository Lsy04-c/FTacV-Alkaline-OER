...
## 69. V5 求解加速触发评估（2026-08-02）

- 触发条件评估：在 Legion 用 S1 同配置（8192 点、32 ppc、hybrid、LSODA、
  mixed_a truth）实测单 forward 耗时分解（3 次取中位）：
  - `forward_current`（ODE）：16.76 s
  - `extract_features`：0.0059 s
  - 合计 16.77 s，**ODE 占 99.97%**
- 并行核查：S1 用 8 workers，95.7 CPU-h / 12.9 wall-h ≈ 92% 利用率，
  并行已基本打满；特征提取只占 0.04%，特征缓存复用收益可忽略。
- 判定：**V5 不启动**。触发条件"现有并行和缓存不足"不成立——并行已到
  本机 8 核瓶颈，缓存维度无收益。唯一实质加速路径为 C++ CN（~5-10×），
  但 Gate A3 已确认 CN 锁相相位偏差（H4-H7），必须重过 A3 特征空间等价
  门；按 roadmap 停止条件（加速改变正式特征、调试成本超过节省），当前
  不启动 V5。
- 证据：Legion profiling 输出见 `/home/lsy/v5_profile.py` 运行结果；
  脚本本地副本 `/tmp/v5_profile.py`。
