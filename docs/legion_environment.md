...
### 并发与工期估算：这台机器约等于 7 个核，不是 16 个

CPU 是 **AMD Ryzen 7 7735H，8 物理核 / 16 线程**。`nproc` 报 16 是逻辑线程数，
**不要按它估算加速比**。2026-09-03 用真实正演（253440 网格点，单次 7.78 s）
实测并行标度：

```
 进程数   墙钟(s)  吞吐(次/s)  相对1路  并行效率
     1     7.67      0.13     1.00x    100%
     2     8.30      0.24     1.85x     92%
     4     8.85      0.45     3.46x     87%
     8    11.74      0.68     5.22x     65%   <- 拐点
    12    14.04      0.85     6.55x     55%
    16    18.12      0.88     6.77x     42%
    24    26.35      0.91     6.98x     29%
```

要点：

* **总吞吐封顶在约 7×**（约 0.9 次正演/秒）。SMT 的两个逻辑线程共享同一个
  物理核的执行单元，对这种计算密集的 ODE 积分只能多榨出约 30%；
* 8 路之后每增加一个进程买到的吞吐迅速衰减，而**单任务墙钟大幅变长**
  （8 路 11.74 s → 16 路 18.12 s，慢 54%）。**MCMC 要等最慢那条链**，
  这个延迟代价是实打实的；
* **工期按总吞吐倒推，不按进程数**。例如 4 链 × 5000 步 = 20000 次正演
  ≈ 6 小时，开 8 路还是 16 路差别不大。

  > 此前文档把"16 核 = 16 进程"写成并发建议，读起来像能拿到 16 倍加速，
  > 实际只有 6.8 倍。**按 16 倍估算会低估工期一倍以上。**

* 内存**不是**瓶颈：16 进程实测约 2 GB / 7.5 GB，24 进程也才约 3 GB。
  宿主有 15.5 GB、WSL 按默认只分到一半，但**没有理由去调大**——
  解决的问题不存在（2026-09-03 实测）。

必须设置 `OMP_NUM_THREADS=1`、`OPENBLAS_NUM_THREADS=1`、`MKL_NUM_THREADS=1`，
否则 BLAS 线程会与进程级并行相乘，把机器压死。
